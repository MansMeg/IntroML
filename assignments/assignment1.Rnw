...
\item (2p) Implement the gradient as a function in R. Below are two examples of how it should work. Note, \texttt{nll\_grad(y, X, theta)} return the gradient of $(1/n) NLL(\theta, \mathbf{y}, \mathbf{X})$. Show that your implementation give the same result as these test cases in your report.
<<echo=FALSE,eval=TRUE>>=
nll_grad <- function(y, X, theta){
c("(Intercept)" = 0.1825, "gre_sd" = -0.0857, "gpa_sd" = -0.0829)
}
@
<<echo=TRUE,eval=TRUE>>=
nll_grad(y, X, theta = c(0,0,0))
@
...
@
<<echo=TRUE,eval=TRUE>>=
nll_grad(y, X, theta = c(-1,0.5,0.5))
@
...
mbsgd <- function(y, X, sample_size, eta, epochs){
  ...
  for(j in 1:epochs){
    ...
    results[j, "epoch"] <- j
    results[j, "nll"] <- # negative log-likelihood
    results[j, -(1:2)] <- theta
  }
  return(results)
}
...
@
\item Take the mean RMSE for the 10 folds.
\end{enumerate}
\item (1p) Now, compute the RMSE for $\lambda=1$ using 10-fold cross-validation.
