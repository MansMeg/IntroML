...
\end{enumerate}
\item Try different learning parameters $\eta$ and run the algorithm for roughly 500 epochs. Visualize your results in two plots per algorithm and choice of $\eta$:
\begin{enumerate}
...
\end{enumerate}
When do the algorithms converge or diverge? Describe your conclusions. Show the code (loop) you use.
\end{enumerate}
