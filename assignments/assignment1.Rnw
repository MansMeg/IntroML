...
\emph{Note!} In general, we should not regularize the intercept (see Hastie et al., 2009, Ch. 3.4). Hence below the gradient and log-likelihood only regularize the parameters \texttt{gre\_sd} and \texttt{gpa\_sd}.
