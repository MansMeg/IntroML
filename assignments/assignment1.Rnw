...
\item Split the data into five folds randomly.
...
y_valfold1 <- y[fold!=1,]
@
\item (5p) Now implement 5-fold cross-validation on the training data.
...
\item Test different values for the hyper parameter $\lambda$. What is the best RMSE you can get with 5-fold cross-validation? What is the $\lambda$ for this value?
\item Now use the best model to do predictions on the test set. What is your RMSE on the testset?
\end{enumerate}
