...
@
\item Visualize the clustering for the worst and the best clustering according to the WPS. \emph{Note!} There might be multiple runs with the same (best) WPS.
...
\begin{enumerate}
\item Implement a function \texttt{pPCA(W, b, sigma2)} that simulate data from the probabilistic PCA model with $h \sim N(0,I)$ and $\sigma^2 = 1$
