...
\item Run 500 simulations of each algorithm and compute the mean of the mean rewards for each of the algorithms. Do this for the greedy, epsilon-algorithm and non-stationary algorithm. Use $\alpha=0.5$, $\alpha=0.9$ and $\epsilon=0.1$. Summarize all the results in a table. In total you should have a four different results. \emph{Note!} The algorithm should run 1000 steps for each of the 500 simulations.
