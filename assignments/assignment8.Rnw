...
%\item How does blei describe the relationship between lda and pca? (max 1 paragraph)
...
\begin{enumerate}
\item Start by loading in the MNIST data in R. See the link above or previous assignments for details.
...
\begin{enumerate}
\item How many weights are used to compute $\mu$ and $\sigma^2$ for the latent variables?
...
For this part of the assignment, Griffith and Steyvers (2004) is the main reference. I would also recommend reading Blei (2012) before starting with the assignment.

We are now going to use a Gibbs sampler to estimate ten different topics occurring in Pride and Prejudice and study where they occur. A tokenized version of the book and a \texttt{data.frame} with stopwords can be loaded as follows:
