...
\begin{enumerate}
\item (1p) Start out by computing the query, key and value matrices for one attention head by implementing it in a function you call \texttt{qkv()}. The function should work like this.
<<eval=FALSE, >>=
...
@
\item (2p) Now, based on your query, key and value, compute the attention of that given attention head for the three chosen tokens.
<<eval=FALSE>>=
...
@
\item (1p) Interpret the attention values. What does the second row mean?
\item (2p) Now we have everything in place for implementing it all as a multi-head attention layer. The layer will take in embeddings and then return a 3-dimensional embedding per word. Run your code on all words in the included example.
<<eval=FALSE>>=
...
\begin{enumerate}
\item (2p) We start out by implementing a simple RNN linear unit that takes the parameters \texttt{W}, \texttt{U}, \texttt{b}, the previous hidden state \texttt{h\_t\_minus\_one} and an input embedding \texttt{x\_t} and return the activation input $a_t$. It should work as follows.
<<eval=FALSE, >>=
...
##
@
\item (1p) Now implement the \texttt{tanh()} activation function.
<<eval=FALSE, >>=
h_t <- activation(a_t)
h_t
##
...
##
@
\item (1p) As the next step, implement the output function and the softmax function. These functions should work in the following way.
<<eval=FALSE, >>=
...
##
@
\item (2p) Now we are ready to implement the full recurrent layer. It should take an input matrix X and the neural network parameters and return the hidden states and the softmax output as follows.
<<eval=FALSE, >>=
...
@
\item (1p) Now, what is the value of the hidden state \texttt{h\_t} for the token \texttt{dog}?
\end{enumerate}
...
\begin{enumerate}
\item (2p) Create a simple ordinary neural network for text classification in Keras using an embedding layer of dimension 16 and a hidden layer with 32 hidden states (and an output layer with one unit). Use a validation split of 20\% observations in the validation set and a batch size of 128. Train your network using rmsprop and use binary cross-entropy as the loss function. Print the model and return your validation accuracy after ten epochs. Most of these settings we will reuse later in the assignments.
\item (1p) Explain what the embedding layer is doing.
\item (1p) Now setup a simple Recurrent Neural Network with 32 hidden units based on the same embedding layer. Print the model and present your result.
\item (1p) The model you have implemented now. Which recurrent network does best describes the model you have now implemented out of Fig. 10.3 and 10.5 in \citet{Goodfellow-et-al-2016}? Motivate why.
\item (1p) Explain the role of the parameters $U$, $W$ in the recurrent net (see Goodfellow et al., 2017 Section 10.2). How large are these parameters in your model specified above, and where are they included in the Keras output?
\item (1p) Now, explain the role of the parameters $V$ (see Goodfellow et al., 2017 Section 10.2). How many parameters are included in $V$, and where are they included in the Keras output?
\item (1p) We are now going to implement an LSTM network. Implement a standard LSTM network with 32 hidden units using Keras. Print your model and your validation accuracy.
\item (1p) Extend your network in ways you think fit. Report at least two other extensions/modifications of your network and why you chose them. Report the Keras model output and validation accuracy.
\item (1p) Reason why we cannot get as good performance as with the MNIST dataset. Is the problem bias, variance, or the Bayes error? Why?
\end{enumerate}
...
\begin{enumerate}
\item (2p) Try to improve on the relatively low classification accuracy and get above 80\% overall accuracy. You are free to elaborate how much you want yourself, but some suggestions are:
\begin{enumerate}
...
\item (1p) Write down the steps you take, the overall accuracy you get along the way, and the final accuracy of your fine-tuned BERT model, both overall and per party.
\item (2p) Reason based on your final confusion matrix on the difficulties for the model. Can the errors be reduced further or not? Why?
\end{enumerate}
