...
\begin{enumerate}
\item As a first step, describe, with your own words what unsupervised learning, and give relevant references to the course literature for this description.
\end{enumerate}
...
\item Simulate values $\mathbf{x}$ of dimension five from a probabilistic PCA model for a given choice of parameters. You are free to choose the parameter values yourself. The latent variables is of should be of dimension two. Note,  the noise as well as the hidden state should be simulated, all other parameters are fixed. Describe all your parameters in matrix notation.
\end{enumerate}
