...
\begin{enumerate}
\item As a first step, describe what a latent variable is.
\item Give an example of a latent variable that we might be interested in.
\end{enumerate}
...
In the next step we will implement the EM algorithm, that is conceptually very similar to k-means klustering.

\subsubsection{Univariare Two Component Normal Mixture}
\label{uni_two_comp}

We are going to implement the EM by testing at the data of \citet{hastie2009elements}. This has been added to the course R package.
...
\item Implement a function called \texttt{e\_uni\_two\_comp(X, theta)} that returns a $n \times 2$ matrix of gamma for each row in \texttt{X}. Note that the row of the gamma should contain the expectation for both components. This function should implement the expectation step in algorithm 8.1 in \citet{hastie2009elements}.
<<eval = FALSE>>=
...
As a last part of the assignment, we will look into the probabilistic Principal Component analysis model. Here, Ch. 14 - 14.1 in Goodfellow et al. is recommended reading.

\begin{enumerate}
\item Setup the model for the probabilistic PCA model.
\item Describe the different parts of the model. What are the parameters? What are the latent variables?
\item Simulate values $\mathbf{x}$ of dimension 3 probabilistic for a given choice of parameters. Choose the parameter values so that the latent variable is of larger dimension than $\mathbf{x}$. Note, only the noise should be simulated, all other parameters you choose yourself. Describe all your paremeter in matrix notation.
\end{enumerate}
