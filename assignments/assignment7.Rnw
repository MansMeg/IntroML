...
For this assignment, you need to have a user account at Open AI to generate an API token. See \href{https://help.openai.com/en/articles/4936850-where-do-i-find-my-api-key} for details on how to get an API token to use.
...
See the \texttt{ropenaiapi} documentation for further information on how to interact with the Open AI API.
...
Now, create 3 different hallucinations with three different strategies (i.e. its not ok to use the same prompt for three incorrect generations) from the chatGPT models (you can choose whichone you like). For each generated hallucination return:
...
\item Why the respose is factually incorrect and weather it is an intrinsic or extrinsic hallucination.
\item Describe the strategy you use and the analysis why the hallucination happended.
\end{enumerate}



\subsection{In-Context Learning for classification}
