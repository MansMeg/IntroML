...
\begin{enumerate}
\item (1p) Use the \texttt{randomForest} R package to fit a random forest regression to the training data using the \texttt{randomForest} function.
<<echo=TRUE,eval=FALSE>>=
library(uuml)
data("Hitters")
Hitters <- Hitters[complete.cases(Hitters),]
...
\item (1p) Train your bagged tree regression model on the training data and predict on the test data. Try the values $B=\{1, 10, 100, 500, 1000\}$. What is the RMSE (using your \texttt{rmse} function) of your predictions on the test set? \emph{Hint!} See Figure 15.1 in \citet{hastie2009elements}.
\end{enumerate}
