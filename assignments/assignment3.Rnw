...
\item What is the benefit of using (negative) log likelihood as loss functions according to Goodfellow et al (2017)? (max 1 paragraph)
%\item What are the benefits with deeper neural networks compared to shallow ones according to Goodfellow et al (2017)? (max 1 paragraph)
...
\item How many parameters does your model have?
...
\item What do you get as classification accuracy? Include the validation error per epoch figure/plot that is automatically generated. \emph{Hint!} The classification accuracy should be around 0.85. \emph{Hint!} In multi-class classification, if the model classify a digit to any other class than the true class, it is a false negative or a false positive. I.e. we can still use the same definition for accuracy, precision and recall as in the binary case in multi-class classification.
\end{enumerate}
\item Now, we are going to play around with the essential concepts in feed-forward networks. Do the steps below, and report the \emph{validation} set accuracy and the validation error per epoch for each step in a figure/plot. Please change this sequantially, so you build a more and more complex network.
\begin{enumerate}
