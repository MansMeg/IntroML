...
\item (2p) Implement a simple feed-forward neural network in R using TensorFlow and Keras. See the introduction in \href{https://github.com/MansMeg/IntroML/tree/master/additional_material/supplementary_r_files/tensorflow_feedforward_neural_network_tutorial.R}{\texttt{tensorflow\_feedforward\_neural\_network\_tutorial.R}}. Implement a neural network with one hidden layer with 16 units and the sigmoid as the activation function for the hidden layer. The output layer should be a softmax. Print your \texttt{keras} model description and include it in your report.
\begin{enumerate}
