...
\item (1p) As the next step, we will implement a convolutional neural network for the CIFAR-10 dataset using \texttt{dataset\_cifar10()}. See the \href{https://tensorflow.rstudio.com/examples/cifar10_cnn}{tutorial} for details on how to load data. Implement a similar CNN as in the tutorial. That is:
<<>>=
## Model: "sequential"
