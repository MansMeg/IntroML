...
\item Describe the explotation - explotation dilemma. (max 1 paragraph)
% Describ a rl policy
% Describe a rl value function
...
R \sim N(q^\star_a, 1)\,,
\]
where $q^\star=(1.62, 1.20, 0.70, 0.72, 2.03)$. Below is an example how it \emph{could} work. Note that since the function return a random value you might get a different result and it might still be a correct implementation. Although, to test, you can run your bandit 1000 times and take the mean (see below). then you should be close to $q^\star$ if your implementation is correct.
<<echo=FALSE>>=
stationary_bandit<- function(a){
  if(a == 1) return(2.99044)
  if(a == 2) return(3.019735)
  if(a == 3) return(0.7099425)
}
<<>>=
set.seed(4711)
stationary_bandit(2)
...
@
\item Implement a greedy policy algorithm that always chooses the greedy action, according to Eq. (2.2) in \citet{sutton2020reinforcement}. The function called \texttt{greedy\_algorithm(Q1, bandit)} should input a vector of initial estimates $Q_1$ of the value of the five actions and a reward function, then run the bandit for 1000 steps and return the rewards $R_t$ for all steps, the mean reward over all the 1000 steps, the value estimates $Q_t(a)$ and the total number of choices of each action $N_t(a)$. Here is an example of how it could work.
<<eval = FALSE>>=
Q1 <- rep(0, 5)
set.seed(4711)
...
\item Run each algorithm 100 times/simulations and compute the mean of the mean rewards (for each simulation) for each of the algorithm. \emph{Note!} The algorithm should run 1000 steps for each of the 100 simulations. Do this for both the stationary and non-stationary bandit. For the non-stationary algorithm, try $\alpha=0.5$ and $\alpha=0.9$ and for the UCB, try $c=0.5$ and $c=2$. Summarize all the results in a table.
\item What are your conclusions from these results? Which algorithm seems to be the best one, and why?
\item For the best and worse algorithm, plot the average reward per step for the 100 runs, such as in Figure 2.2. in \citet{sutton2020reinforcement}. Use the output $R_t$ from your implementations.
\end{enumerate}

\section{Markov Decision Processes}

Now we are going to implement a Markov Decision Process (MDP) where the agent makes a decision, $A_t$ based on the current state $S_t$ and a reward $R_t$ that will, in turn, return a new state $S_{t+1}$ and a reward $R_{t+1}$. In this part of the assignment, we are going to use the recycling robot example \citep[Example 3.7 in ][]{sutton2020reinforcement}.
...
\begin{enumerate}
\item Implement the UCB algorithm as a function called \texttt{ucb\_algorithm(Q1, bandit, c)}. \emph{Hint!} If $N_t(a) = 0$, then $a$ should be considered as the maximizing action (i.e. should be chosen).
<<eval = FALSE>>=
set.seed(4711)
Q1 <- rep(0, 5)
...
@
\item Implement the gradient bandit algorithm as a function called \texttt{gradient\_bandit\_algorithm(bandit, alpha)}. Initialize all $H_t(a) = 1$. The algorithm should print the final preference and the probability of each action instead of $Q_t$.
<<eval = FALSE>>=
set.seed(4711)
...
@
\item Test to run the gradient bandit with $\alpha = 0$. What happens and why?
\item Run the gradient bandit and the UCB algorithms 100 times/simulations and compute the mean of the mean rewards (for each simulation) for each of the algorithm. \emph{Note!} The algorithm should run 1000 steps for each of the 100 simulations. Use both the stationary and non-stationary bandit. For the UCB, try $c=0.5$ and $c=2$ and for the gradient bandit $\alpha = 0.1$ and $\alpha = 1$. Summarize all the results in a table and compare the results with the previous bandit algorithms. Which algorithm performs the best?
\end{enumerate}
