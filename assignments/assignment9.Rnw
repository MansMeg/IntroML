...
\item Describe the exploitation-exploitation dilemma. (max 1 paragraph)
...
\item Pot the average reward per step for the 100 runs for the worst and best algorithm, such as in Figure 2.2. in \citet{sutton2020reinforcement}. Use the output $R_t$ from your implementations.
\end{enumerate}
