...
\[
\mathbf{d} = \{(y_i, \mathbf{x}_i), i = 1, \ldots, n\} \,.
\]
...
\item In probabilistic models we can use the log-likelihood ($\log p(x)$), or {\color{uured} perplexity} (a function of the log-likelihood).
\begin{itemize}
...
\[
\gamma_{i,k} = \mathbb{E}_{z_i} (\mathcal{L}_{\text{full}}|\theta_k, y_i)\,.
\]
\pause
...
\begin{frame}{Why probabilistic PCA matters}
pPCA:
\begin{itemize}
\item Handles missing data naturally
\item Allows likelihood-based model comparison
\item Can be embedded in mixture models
\item Generalizes to Bayesian PCA and factor models
\end{itemize}
