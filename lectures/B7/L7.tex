...
\begin{itemize}
\item \uured{Deep} Autoencoder: An autoencoder with {\color{uured} multilayer neural networks} as encoder and decoder
\begin{itemize}
...
\end{itemize}\pause
\item \uured{Optimization target}: \\Maximize the variational lower bound or {\color{uured} evidence lower bound (ELBO)}
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Use cases:
    \begin{itemize}
        \item Create \uured{features} for supervised models\pause
        \item Integrated in neural networks for \uured{model efficient learning} (?)\pause
