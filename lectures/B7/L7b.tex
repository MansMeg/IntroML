...
\begin{frame}{Decoding (word generation): Top-$p$}
...
\end{itemize}
\pause
\item Still BERT is competetive in many ML tasks (e.g. see Sargeant et al 2025)% (see Ollion et al, 2023)
\end{itemize}
\end{frame}
...
\end{itemize}
\pause
\item \uured{Goal}: improve factuality and reduce hallucinations.
\item Instead of relying only on parametric memory (weights),
the model conditions on \uured{retrieved documents}.
\end{itemize}
...
\item knowledge bases, APIs, logs, Wikipedia
\end{itemize}
\item Knowledge can be updated \uured{without retraining} the LLM.
\end{itemize}
