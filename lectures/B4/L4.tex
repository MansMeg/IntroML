...
\begin{itemize}
\item Learning representations in $P_1$ will help generalizations in $P_2$
\item Main idea: transfer learned representations to new settings where they might be useful
\item "{\color{uured} Transfer knowledge} between problems"
\item Transfer/reuse {\color{uured} learned weights}\pause
\item A Bayesian perspective: A strong prior
\item Use (large) pre-trained models for smaller problems
\item One of the main reasons for the success of (convolutional) neural networks\pause
\item The Risk: Catastophic forgetting
\item Two types of transfer learning in Neural Networks:
\begin{itemize}
\item Feature extraction
\item Fine Tuning
\end{itemize}
\item \uured{Domain adaptation}: Same problem but at different input dataset (e.g. language models for legal/medical/political data)
\item Concept drift: Similar problem
\item Previously popular with unsupervised pre-training. Now mainly resused models.
\end{itemize}
...
\item Setup your baseline \\(establish a working end-to-end pipeline as soon as possible)
\item Diagnose your networks performance
...
\item Set up the metric based on the overall target of the system! This may need multiple metrics.\pause %
...
\item It is not uncommon that manual curation is faster and easier in some settings.
\end{itemize}
}
...
\begin{itemize}
\item Start with the most simple possible model (logistic regression)
\item Start from previous approaches
\item Start with a simple neural network that is common in the domain/defaults (CNN for images, Adam as optimizer)
\end{itemize}
}
...
\frame{\frametitle{3. Diagnosing and improving your model: More data}

\begin{itemize}
\item Gather more data: Usually a good solution
\item Avoid focus on the "best" algorithm and the "ML-sickness"
\item If high loss in training: The training data is not yet used fully
\item If high loss on test: The quality of the training data can be poor. This means that the Bayes error is larger.
\item If low training error, but high test error: Probably more data is a good way forward. If costly or difficult with more data -> regularization

\end{itemize}
}

\frame{\frametitle{Learning curve to assess if you need more data}
...
\begin{itemize}
\item General idea: Increase data and model capacity until target is reached.
\item Adapt hyperparameters to get \uured{optimal capacity}
\item Goal: The primary goal of manual hyperparameter search is to adjust the effective capacity of the model to match the complexity of the task.
\item Ideally, each marginal hyperparameter has a U-shaped form
\item Step 1: Get good training error. Tune learning rate. U-shaped for training error. Neural network performs best when training error is low.
\item Step 2: Tuning hyperparameters (regularization) requires monitoring both training and test error to diagnose whether your model is overfitting or underfitting
\item Best performance: Larger model that is regularized well.
\item End goal: Good performance on test set
\item With low training error you can always improve by adding more data
...
\item \uured{What is a hyperparameter in a neural network?}
\item Grid search:
\begin{itemize}
...
\item random search can be exponentially more efficient
\item reduce the error faster
\end{itemize}
\end{itemize}
}
...
\begin{itemize}
\item Visualize the model predictions
\item Analyze the worst errors: \uured{why?}
\item Use train and test error as a diagnostic: \uured{Can you overfit the data?}
\item Use test suites: Can you get known results on a toy data. Both training error and derivatives
\item Monitor the gradients
