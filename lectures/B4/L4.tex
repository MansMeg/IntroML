...
\begin{itemize}
\item It takes a long time to run the models this week. Start early!
\end{itemize}
\end{frame}
...
\item 2D-grids (matrix): Gray-scale Images (pixels)\pause
\item 3D-grids (3D tensor): Color Images (pixels and channels)\pause
\item 4D-grids (4D tensor): Color Video (pixels, channels, frames, time)
\end{enumerate}
\end{itemize}
...
\item Spatial and hierarchical correlation structures
\end{itemize}
}
...
\begin{frame}{Why Not Use FFNN for Images?}

\begin{itemize}

    \item \uured{Parameter explosion:}
    A fully-connected layer ignores image structure.
    \[
        256 \times 256 \times 3 \approx 200{,}000 \text{ inputs}
    \]
    A single hidden layer with 1{,}000 units would require
    \[
        200{,}000 \times 1{,}000 = 200 \text{ million parameters}.
    \]

    \pause

    \item \uured{Locality is ignored:}
    Nearby pixels are strongly correlated.
    FFNNs treat all pixels as independent inputs.

    \pause

    \item \uured{Missing invariances:}
    Images contain translation/shift invariance.
    MLPs must \emph{learn} this from data instead of building it in.

    \pause

    \item \uured{\textbf{What CNNs do intuitively}}:
    \begin{itemize}
        \item Look at small local regions (receptive fields)
        \item Reuse the same filter across the image (weight sharing)
        \item Build hierarchical features: edges $\rightarrow$ textures $\rightarrow$ shapes
    \end{itemize}
...
\caption{The representations of a cat (Chollet and Allaire, 2018, Fig 5.2)}
\end{figure}
\pause
...
\[
Y(i,j) = (X * K)(i,j) = \sum_{m} \sum_{n} X(m, n) K(i-m, j-n)
\]
\pause
...
\begin{enumerate}
\item $X(i,j)$: Input (2D)
\item $K(i,j)$: Kernel, \uured{filter}, "feature" (2D)
\item $Y(i,j)$: Output, \uured{feature map} (2D)
\end{enumerate}
\end{itemize}
}
...
\item More kernels/filters higher up in the network
\item A classification head (usually a \uured{feed-forward} neural network)
\end{enumerate}
\pause
\item Benefits:
\begin{enumerate}
...
\item Necessary to {\color{uured} keep size} of feature maps
\end{itemize}
...
\caption{Padding and valid edge handling (Chollet and Allaire (2018), Fig. 5.5, 5.6)}
\end{figure}
}
...
\caption{Strides (Chollet and Allaire (2018), Fig. 5.5, 5.6)}
\end{figure}
}

\begin{frame}{How Convolutions Change Tensor Shapes}

\begin{itemize}

    \item Input tensor shape:
    \[
        X \in \mathbb{R}^{H \times W}
    \]
    where $H$ is the height and $W$ is the width of the image.

    \item Convolution parameters:
    \begin{itemize}
...
\begin{frame}{Example: Computing Output Size}

\begin{itemize}

    \item Given an input image:
    \[
        32 \times 32 \times 3
    \]

    \item Convolution layer parameters:
    \begin{itemize}
        \item Kernel size: $3 \times 3$
        \item Padding: $p = 1$
        \item Stride: $s = 1$
        \item Number of filters: $C_{\text{out}} = 16$
    \end{itemize}
    ...
    \]
    \[
        W' = \frac{32 - 3 + 2}{1} + 1 = 32.
    \]

    \pause

    \item Final output tensor:
    \[
        32 \times 32 \times 16.
    \]
...
\end{enumerate}
\item Example: a 1 megapixel, single channel, image ($1000 \times 1000$ pixels)
\begin{enumerate}
...
\item $\mathbf{X W}$ is the convolutional feature map ($X * W$)\pause
...
\begin{itemize}
\item We take a function $f$ that returns one value per pooling kernel\pause
\item Most commonly $f=\max$
\item {\color{uured} Max Pooling}: Take the maximum value in the pooling kernel\pause
\item Pooling has \uured{no trainable parameters}\pause
...
\caption{Strides ("Computer Science" at Wikipedia)}
\end{figure}
}
...
\begin{frame}{Pooling Layers: Pros and Cons}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Reduces spatial resolution → computational savings
        \item Helps prevent overfitting by reducing model capacity
    \end{itemize}
    ...
    \item \uured{Cons}
    \begin{itemize}
        \item Discards fine-grained spatial information
        \item Can hurt performance on tasks requiring precise localization
              (e.g., segmentation, object detection)
        \item Often replaced or reduced in modern architectures
              (e.g., strided convolutions)
    \end{itemize}

    \vspace{5mm}

    See \emph{Benefits of Max Pooling in Neural Networks: Theoretical
and Experimental Evidence} by Matoba et al. (2023) for more details.
...
\caption{Data Augmentation (Chollet and Allaire, 2018, Fig 5.10)}
\end{figure}
\pause
\begin{itemize}
...
\caption{Using convnets as base for feature extraction (Chollet and Allaire, 2018, Fig 5.12)}
\end{figure}
...
\caption{Finetuning a convolutional base (Chollet and Allaire, 2018, Fig 5.15)}
\end{figure}
...
\item \uured{Catastrophic forgetting} \pause
...
\begin{frame}{Transfer Learning: Practical Strategy}

\begin{itemize}

    \item \uured{Step 1: Load a pretrained CNN}
    (e.g., ResNet, EfficientNet) trained on ImageNet.

    \pause

    \item \uured{Step 2: Freeze the convolutional base}
    Train only the new classification head

    \pause

    \item \uured{Step 3: Train the head first}
    Use a higher learning rate (e.g., $10^{-3}$).

    \pause

    \item \uured{Step 4: Unfreeze top convolutional blocks}
    Fine-tune with a \emph{small} learning rate (e.g., $10^{-5}$)
    to avoid destroying pretrained features.

    \pause

    \item \emph{When it works best:}
    \begin{itemize}
        \item You have limited data
        \item Your task is visually similar to pretrained data (e.g. ImageNet)
        \item You want fast convergence and stable training
    \end{itemize}

    \item \emph{When not to fine-tune:}
