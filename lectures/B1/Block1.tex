...
\item An individual computer assignment (approx. 12-16h). Deadline Sundays 23.59.
\end{itemize}
...
\item Acknowledgements: M{\aa}ns Thulin, Josef Wilzén\pause
...
\item Ian Goodfellow and Yoshua Bengio and Aaron Courville (2017). \emph{Deep Learning}. Available at \\\texttt{https://www.deeplearningbook.org/}
...
\begin{enumerate}
\item All labs need to be passed\pause
\item The grade are set based on the project report and the review.\pause
\item In borderline cases, I will look at:
\begin{enumerate}
\item How much have you been helping others on Slack (public channel)?
\item Any improvements/bug fixes as Pull Requests?
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
\item Training is usually done by minimizing the loss/cost function $L(\theta)$ for $\theta \in \mathbf{R}^P$.
