...
\begin{center}
\begin{tabular}{ l | c | c }
  & $\hat{f}(x)=\text{ham}$ & $\hat{f}(x)=\text{spam}$\\
  \hline
  $y=\text{ham}$ & 515 & 91 \\
  $y=\text{spam}$ & 85 & 569 \\
  \hline
...
\item Precision(ham): $\frac{515}{515+85}\approx 0.86$
\item Recall(ham): $\frac{515}{515+91}\approx 0.85$
\item $F_1$(ham): $\frac{2 \cdot 0.85 \cdot 0.86}{0.85 + 0.86}\approx 0.855$
\end{enumerate}

\pause

In this example, we let $\hat{y}_i=\text{ham}$ whenever $\hat{\pi}_i>0.5$.
...
\begin{center}
\begin{tabular}{ l | c | c }
 {\color{uured}$\alpha=0.5$} & $\hat{f}(x)=\text{ham}$ & $\hat{f}(x)=\text{spam}$\\
  \hline
  $y=\text{ham}$ & 515 & 91 \\
  $y=\text{spam}$ & 85 & 569 \\
  \hline
...
\end{center}
\pause
Now let $\alpha=0.3$ instead, so that we are more prone to say that $\hat{y}=\text{spam}$:
\begin{center}
\begin{tabular}{ l | c | c }
  {\color{uured}$\alpha=0.3$} & $\hat{f}(x)=\text{ham}$ & $\hat{f}(x)=\text{spam}$\\
  \hline
  $y=\text{ham}$ & 462 & 144 \\
  $y=\text{spam}$ & 38 & 616 \\
  \hline
...
\item Precision(ham): $\frac{462}{462+38}\approx 0.92$
\item Recall(ham): $\frac{462}{462+144}\approx 0.76$
\item $F_1$(ham): $\frac{2 \cdot 0.92 \cdot 0.76}{0.92 + 0.76}\approx 0.83$
\end{enumerate}
...
\item Precision(ham): $\frac{1001}{1001+0}\approx 1.0$
\item Recall(ham): $\frac{1001}{1001+17}\approx 0.99$
\item $F_1$(ham): $\frac{2 \cdot 1 \cdot 0.99}{0.99 + 1}\approx 0.99$
\pause
\item $F_1$ for spam: $\frac{2 \cdot 0 \cdot 0}{0 + 0}$ Not defined.
...
\begin{itemize}
\item The main error of interest - \uured{generalization error}
...
\[
\frac{1}{N} \sum_{i=1}^{N} \text{Err}(x_i)
=
\underbrace{\sigma_{\epsilon}^2}_{\text{irreducible}}
+
\underbrace{\frac{1}{N} \sum_{i=1}^{N} \big(f(x_i) - \mathbb{E}[\hat{f}(x_i)]\big)^2}_{\text{bias}^2}
+
\underbrace{\frac{p}{N} \sigma_{\epsilon}^2}_{\text{variance}}
\]
where $N$ is the number of observations and $p$ the number of covariates.
...
\begin{figure}[h]
\caption{Test (red), training (blue), and model complexity (Hastie et al, 2009, Figure 7)}
\centering
...
\begin{itemize}
\item We want to estimate \uured{the generalization error $\text{Err}$} for different models and to \uured{choose the best model} where
\begin{align*}
...
Further, assume the true model is
\[
...
\begin{figure}[h]
\caption{MSE and misscoverage with $\text{Err}_{XY} = \text{Err}_\mathcal{T}$ (Bates et al, 2022)}
\vspace{-5mm}
\centering
