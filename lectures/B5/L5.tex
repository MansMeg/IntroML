...
\item Transformers
\item Encoder-based (BERT-type) models
\end{itemize}
\end{frame}
...
\begin{itemize}
\item One lecture later this week on word embeddings (Väinö Yrjänäinen) and encoder models (Hannes Waldetoft)
\end{itemize}
...
\caption{Representing words as word embeddings (Chollet and Allair, 2018, Fig. 6.1)}
\end{figure}
...
\begin{itemize}
\item A word type represents {\color{uured} meaning in a low-dimensional semantic space}
...
\begin{itemize}
\item \uured{Static embeddings} (word2vec, GloVe) assign \emph{one vector per word type}.
\pause
\item \uured{Contextual embeddings} (BERT/Transformers) create \emph{context-dependent vectors} per token.
\end{itemize}
...
\end{itemize}
\pause
\item Examples of applications (but gets rarer and rarer due to Transformers):
\begin{itemize}
...
\[
W \in \mathbb{R}^{H \times H}, \qquad
U \in \mathbb{R}^{H \times D}, \qquad
V \in \mathbb{R}^{D \times H} \; \text{(if needed)}.
\]
...
\item Behind the recent progress in NLP: BERT, GPT-series, Gemini etc.
\pause
\item De-facto \uured{standard} in industry and academia
\pause
\item Four benefits:
\begin{itemize}
...
\item Output: \uured{contextualized} embeddings
\end{itemize}
\item Decoder:
\begin{itemize}
...
\begin{frame}{Self-Attention Example: Setup}

\textbf{Sentence:} ``The cat sat''

\medskip
Use simple 2D embeddings:

\[
\text{the} = \begin{bmatrix}1 \\ 0\end{bmatrix}, \quad
\text{cat} = \begin{bmatrix}0 \\ 1\end{bmatrix}, \quad
\text{sat} = \begin{bmatrix}1 \\ 1\end{bmatrix}
\]

We compute self-attention for the token \textbf{``cat''}.

\medskip
Projection matrices (toy example):
...
\begin{frame}{Step 1: Compute Q, K, and V}

\textbf{Query for ``cat'':}
\[
Q_{\text{cat}} = W_Q \cdot \text{cat}
...
\textbf{Keys:}
\[
K_{\text{the}} = \begin{bmatrix}1 \\ 0\end{bmatrix}, \quad
K_{\text{cat}} = \begin{bmatrix}1 \\ 1\end{bmatrix}, \quad
...
Values:
\[
V_{\text{the}} = \begin{bmatrix}1 \\ 1\end{bmatrix}, \quad
V_{\text{cat}} = \begin{bmatrix}0 \\ 1\end{bmatrix}, \quad
V_{\text{sat}} = \begin{bmatrix}1 \\ 2\end{bmatrix}
\]

\end{frame}

\begin{frame}{Step 2: Attention Scores and Weights}

\textbf{Dot-product scores for ``cat'':}
\[
\left[
Q_{\text{cat}}^\top K_{\text{the}},
Q_{\text{cat}}^\top K_{\text{cat}},
Q_{\text{cat}}^\top K_{\text{sat}}
\right]
=
[0,\; 1,\; 1]
\]
...
\textbf{Softmax weights:}
\[
\text{weights} = \mathrm{softmax}([0,\,1,\,1])
= [0.21,\; 0.39,\; 0.39]
\]

\end{frame}

\begin{frame}{Step 3: Output Representation for ``cat''}

\textbf{Weighted sum of values:}

\[
\text{Output}_{\text{cat}}
= 0.21 \cdot V_{\text{the}}
+ 0.39 \cdot V_{\text{cat}}
+ 0.39 \cdot V_{\text{sat}}
\]

\[
= 0.21\begin{bmatrix}1\\1\end{bmatrix}
+ 0.39\begin{bmatrix}0\\1\end{bmatrix}
+ 0.39\begin{bmatrix}1\\2\end{bmatrix}
=
\begin{bmatrix}0.60 \\ 1.59\end{bmatrix}
\]
...
\end{figure}
\vspace{-7mm}
\uured{Attention weight} = how much word $i$ focuses on word $j$ when computing its contextual representation.
\end{frame}
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item Byte-pair encoding (GPT, RoBERTa)
        \item WordPiece (BERT)
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    ...
    \pause
    \item Schuster and Nakajima (2012) present the WordPiece model
    \pause
...
\pause
Transformer attention is \uured{order-invariant}, so positional encodings inject order information.
...
\begin{itemize}
\pause
\item Common models are BERT, RoBERTa and ModernBERT
\end{itemize}
\end{frame}
...
\pause
\item {\color{uured} (Still) State-of-the-Art} in many text prediction tasks, such as
\begin{itemize}
    \item Named-Entity Recognition
    \item Text Classification
\end{itemize}
\pause
\item Many flavors, such as RoBERTa, AlBERT, ModernBERT etc.
\pause
...
\begin{frame}{BERT (continued)}

\begin{itemize}
\item Available in English, Swedish, and many other languages (e.g.\ The National Library)
\pause
\item Note!
\begin{itemize}
    \item BERT = encoder-only (no generation)
    \item As we will see, GPT = decoder-only (generation)
\end{itemize}
\pause
\item And again, I rely on Alammar (2018): \uured{The Illustrated BERT}
\end{itemize}
