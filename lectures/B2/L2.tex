...
\item Let
\[
f_\text{boost}(x) = \sum_b \hat{T} (x | \Theta_b)
\]
\pause
\item We want to minimize the regularized loss
\[
L = l(y, f_\text{boost}(x)) + \sum_b \Omega(\hat{T} (x_i | \Theta_b))
\]
\pause
where
\[
\Omega(\hat{T} (x_i | \Theta_b)) = \lambda_1 L + \lambda_2 ||\gamma||^2
\]
\end{itemize}
