...
\frame{\frametitle{The Hype: Deep Learning}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{fig/dl-dev2.png}
\caption{Benchmark saturation over time for popular
benchmarks, normalized with initial performance at mi-
nus one and human performance at zero. (Kiela et al., 2021)}

\end{figure}
}


\begin{frame}{The Hype: Large Language Models}

\begin{itemize}
    \item \uured{GPT-3 (2020)}
    175B parameters; breakthrough in general language capabilities.
\pause
    \item \uured{2021–2022: Scaling + Instruction Tuning : ChatGPT}
    \begin{itemize}
        \item Models such as GPT-3.5, Gopher, Chinchilla, PaLM.
        \item Emergence of instruction-following (ChatGPT)
    \end{itemize}
    \pause

    \item \uured{2023: GPT-4 Era}
    \begin{itemize}
        \item Multimodal models (image+text)
        \item Open models: LLaMA, Falcon
        \item Focus on safety and alignment
    \end{itemize}
    \pause

    \item \uured{2024: Open-Weights Explosion}
    \begin{itemize}
        \item LLaMA-2/3, Mistral, Mixtral (MoE)
        \item Efficient inference and training.
        \item Training data reached internet-size.
    \end{itemize}
    \pause

    \item \uured{2025: Multimodal Models}
    \begin{itemize}
        \item Native multimodal architectures (audio–vision–text).
        \item Agents: reasoning + planning + tool execution.
        \item GPT-5 released
    \end{itemize}
...
\begin{itemize}
\item Although - (Deep) Neural Networks are not a silver bullet\pause
\item Remember the {\color{uured} Bayes error}\pause
\item Sometimes a linear regression (or Random Forest) is enough
\end{itemize}
...
\item {\color{uured} Transformer Neural Networks}: Text/Audio/Vision
...
Layers\pause, neurons\pause, input\pause, output\pause, weights\pause, bias\pause, architecture
...
\begin{itemize}
    ...
    \item A feed-forward neural network with \uured{one hidden layer} and a
    \uured{non-polynomial activation} (e.g., sigmoid, tanh, ReLU) can approximate
    \uured{any continuous function} on a compact set $K \subset \mathbb{R}^n$.
    % A compact set is a finite, closed, “well-behaved" region in space.
    \pause
    \item Formally, for any continuous function $f : K \to \mathbb{R}^m$
    and any $\varepsilon > 0$, there exists a one-hidden-layer network
    \[
        g(x) = C\,\sigma(Ax + b)
    \]
    such that
    ...
    \item Interpretation: The \uured{worst-case error} between the network $g$ and the
    target function $f$ on the entire set $K$ can be made \uured{arbitrarily small}
...
\frame{\frametitle{Universal Approximation Theorem, but...}

\begin{itemize}
\item Neural networks are \uured{universal function approximators}, but...\pause
\begin{itemize}
\item No guarantee we can \uured{learn} the network\pause
\item No guarantee that it will \uured{generalize}\pause
\item No indication of how large the network needs to be
\end{itemize}
\end{itemize}
...
\item Art rather than science, however...\pause
\begin{enumerate}
\item \uured{Start}: get training error to zero\pause
\item \uured{Regularize}: get validation error to minimum
\end{enumerate}
\item Specialized approaches (Bayesian optimization)\pause
\item Grid search (combinatorial explosion)
\begin{itemize}
\item Really bad if many hyperparameters has a little effect
\item If we have 5 irrelevant parameters we try 3 values for: 125 training per relevant run
\end{itemize}
\item Instead use...\pause \uured{Random search}
\end{itemize}
...
\begin{itemize}
\item Reduce training error but \uured{improve generalization error}\pause
...
\]
where $J(\theta)$ is the cost function, $\theta$ all weights and biases, and $\alpha \Omega(\theta)$ is the penalty for the weight matrices and where $\alpha$ is the strength of the penalty.
\end{itemize}
...
\item Let
\[
\Omega_1 (\theta) = \sum_i \sum_j |\theta|_{i,j}\,,
\]
and
\[
\Omega_2 (\theta) = \sum_i \sum_j \theta^2_{i,j}\,,
\]
be the $L_1$ and $L_2$ regularization respectively.
\item We can then get the cost function
\[
\tilde{J}(\theta) = J(\theta) + \sum_l \alpha_l \Omega_2(\theta_l)\,,
\]
\end{itemize}
...
\begin{itemize}
\item Let's define the cost function for the weights $w$ as
\begin{align*}
...
\end{itemize}
\item The dropout probability (dropout rate) is typically 0.2 for input nodes and 0.5 for hidden nodes
\item Forces the network to
\begin{itemize}
\item not rely on individual nodes
\item spread out the weights over more nodes
\end{itemize}
\item Can be seen as an ensemble method
\end{itemize}
}
...
\begin{itemize}
\item Adam (the main optimizer today)
\item RMSprop
\end{itemize}\pause
\item To compute $\hat{\nabla} J(\theta_{t-1})$:
\begin{itemize}
...
\begin{itemize}
    ...
    \item Backpropagation provides an efficient procedure for computing all
    parameter gradients $\nabla J(\theta)$ in a feed-forward neural network.

    \item Uses the chain rule to propagate the error backward through layers:
    \[
        \frac{\partial J}{\partial W_l}
        = h_{l-1}^\top \, \delta_l,
    \qquad
        \frac{\partial J}{\partial b_l}
        = \delta_l,
    \]
    where $\delta_l$ is the error signal at layer $l$.

    \item The algorithm consists of two passes:
    \begin{itemize}
        ...
        \item \textbf{Backward pass:} compute error signals
        \[
            \delta_l = \bigl(\delta_{l+1} W_{l+1}^\top \bigr) \odot g_l'(z_l),
        \qquad
            z_l = h_{l-1}^\top W_l + b_l,
        \]
    ...
    \item Enables efficient gradient-based optimization even for networks
    with many layers and millions of parameters.

\end{itemize}
\end{frame}

\begin{frame}{Forward Pass: Simple Example}
\begin{itemize}

\item Consider a network with one hidden layer ($2$ units, we skip biases for simplicity) and ReLU activation:
    \[
        h_1 = g_1(x^\top W_1), \qquad
        \hat{y} = w^\top h_1.
    \]
    \item Let
    \[
...
%        b_2 = 0.
    ...
    \item Compute hidden pre-activation:
    \[
        z_1 = x^\top W_1
        =
        \begin{bmatrix}
...
\begin{frame}{Backward Pass: Simple Example}
\begin{itemize}
    ...
    \[
        ...
        \qquad y = 0,
        \qquad
        \frac{\partial J}{\partial \hat{y}} = \hat{y} - y = -8.
    \]

    \item Output layer gradients:
    \[
        \frac{\partial J}{\partial w}
        = \frac{\partial J}{\partial \hat{y}} \cdot \frac{\partial \hat{y}}{\partial w}
        = h_1 (\hat{y} - y)
        =
        \begin{bmatrix}
        0 \\ -40
    ...
    \item Backpropagated error signal:
\[
\delta_1
:=
...
=
(\hat{y} - y)\, w \;\odot\; g'_{\text{ReLU}}(z_1).
\]
...
\end{bmatrix}.
\]
    \item Gradients for first layer:
    \[
...
\subsection{Difficulties in Neural Network Optimization}
% TODO: Add
